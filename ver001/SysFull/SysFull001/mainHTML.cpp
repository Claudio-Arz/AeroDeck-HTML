...
<div class="grid-container">
  ...
  <div class="grid-item airspeed" style="grid-row: 1; grid-column: 1;">
    <div id="container-airspeed" style="display:inline-block;position:relative;width:100%;height:100%;box-shadow:0 0 20px 10px #000a;border-radius:50%;background:#1d3958;border:4px solid #ff0000;overflow:hidden;">
      <img src="https://1drv.ms/i/c/976676efea6bc5cc/IQRdNEOaXUI4SbuC53tSbYNCAUxWlKMTEPJ_IY05csbj6Mo?width=240&height=240" style="width:100%;height:100%;position:absolute;top:0;left:0;z-index:1;pointer-events:none;" alt="Indicador AirSpeed">
      <div id="needle-airspeed" class="needle" style="border-bottom: 90px solid #ffcc33; z-index:2;"></div>
      <div class="needle-center" style="background:radial-gradient(circle at 30% 30%, #e92084, #a20202);border:2px solid #222;z-index:3;"></div>
      <div id="airspeed-value" style="position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);font-size:24px;width:80px;text-align:center;z-index:20;background:rgba(8,32,50,0.85);border-radius:10px;pointer-events:none;color:#fff;box-shadow:0 0 8px #000;">--</div>
    </div>
  </div>
  ...
  <div class="grid-item slider-vertical" style="grid-row: 2; grid-column: 7;">
    ...
    <button id="start-btn" style="height:48px;font-size:18px;padding:8px 12px;margin-top:12px;border-radius:8px;">Start</button>
  ...
  <!-- Pitch y Roll juntos en Row 1, Col 6 -->
  <div class="grid-item slider-vertical" style="grid-row: 1; grid-column: 6;">
    <div style="display: flex; flex-direction: row; gap: 16px; align-items: flex-end; justify-content: center; width: 100%;">
  ...
  <!-- AirSpeed Slider: Row 1, Col 8 -->
  <div class="grid-item slider-vertical" style="grid-row: 1; grid-column: 8;">
    <div style="text-align:center;font-weight:bold;color:#fff;margin-bottom:4px;">AIRSPEED</div>
    <input type="range" min="40" max="200" value="40" class="slider" id="airspeed-slider-col8" orient="vertical">
    <div class="slider-value" id="airspeed-slider-value-col8">40</div>
  </div>
</div>
...
// === Variables de estado visual para animación suave ===
let instrumentState = {
  airspeed: 40,
  rpm: 0,
  fuelflow: 0,
...
let instrumentTarget = { ...instrumentState };

function animateInstrument(key, updateFn) {
  const speed = 0.2; // 0.0-1.0, mayor = más rápido
  function step() {
    let current = instrumentState[key];
    let target = instrumentTarget[key];
    let delta = target - current;
    if (Math.abs(delta) > 0.1) {
      instrumentState[key] += delta * speed;
      updateFn(instrumentState[key]);
      requestAnimationFrame(step);
    } else {
      instrumentState[key] = target;
      updateFn(target);
    }
  }
  step();
...
function updateRPMInstrument(rpm) {
  if (userSliding.rpm) return;
  instrumentTarget.rpm = rpm;
  animateInstrument('rpm', function(val) {
    const rpmNeedle = document.getElementById("needle-rpm");
    const rpmValue = document.getElementById("rpm-value");
    if (rpmNeedle) {
      const minValue = 0, maxValue = 3000, minAngle = 30, maxAngle = 330;
      let angle = minAngle + ((val - minValue) * (maxAngle - minAngle)) / (maxValue - minValue);
      rpmNeedle.style.transform = `translate(-50%, -100%) rotate(${angle}deg)`;
    }
    if (rpmValue) rpmValue.textContent = Math.round(val);
    const rpmSlider = document.getElementById("rpm-slider");
    const rpmSliderValue = document.getElementById("rpm-slider-value");
    if (rpmSlider) { rpmSlider.value = val; rpmSliderValue.textContent = Math.round(val); }
  });
}
function updateFuelFlowInstrument(fuelFlow) {
  if (userSliding.fuelflow) return;
  instrumentTarget.fuelflow = fuelFlow;
  animateInstrument('fuelflow', function(val) {
    const ffNeedle = document.getElementById("needle-fuelflow");
    const ffValue = document.getElementById("fuelflow-value");
    if (ffNeedle) {
      const minValue = 0, maxValue = 20, minAngle = 30, maxAngle = 330;
      let angle = minAngle + ((val - minValue) * (maxAngle - minAngle)) / (maxValue - minValue);
      ffNeedle.style.transform = `translate(-50%, -100%) rotate(${angle}deg)`;
    }
    if (ffValue) ffValue.textContent = parseFloat(val).toFixed(1);
    const ffSlider = document.getElementById("fuelflow-slider");
    const ffSliderValue = document.getElementById("fuelflow-slider-value");
    if (ffSlider) { ffSlider.value = val; ffSliderValue.textContent = parseFloat(val).toFixed(1); }
  });
}
function updateVSInstrument(vs) {
  if (userSliding.vs) return;
  instrumentTarget.vs = vs;
  animateInstrument('vs', function(val) {
    const vsNeedle = document.getElementById("needle-vs");
    const vsValue = document.getElementById("vs-center-value");
    if (vsNeedle) {
      const minValue = -127, maxValue = 127, minAngle = -60, maxAngle = 60;
      let angle = minAngle + ((val - minValue) * (maxAngle - minAngle)) / (maxValue - minValue);
      vsNeedle.style.transform = `translate(-50%, -100%) rotate(${angle}deg)`;
    }
    if (vsValue) vsValue.textContent = Math.round(val);
    const vsSlider = document.getElementById("vs-slider");
    const vsSliderValue = document.getElementById("vs-slider-value");
    if (vsSlider) { vsSlider.value = val; vsSliderValue.textContent = Math.round(val); }
  });
}
function updateAttitudeInstrument() {
  if (userSliding.pitch || userSliding.roll) return;
  instrumentTarget.roll = parseFloat(rollSlider.value);
  instrumentTarget.pitch = parseInt(pitchSlider.value);
  animateInstrument('roll', function(val) {
    if (fondoImg) fondoImg.style.transform = `rotate(${val}deg)`;
    if (ballImg) ballImg.style.transform = `rotate(${val}deg) translateY(${instrumentTarget.pitch}px)`;
    if (dialImg) dialImg.style.transform = `rotate(${val}deg)`;
  });
  animateInstrument('pitch', function(val) {
    if (ballImg) ballImg.style.transform = `rotate(${instrumentTarget.roll}deg) translateY(${val}px)`;
  });
}

// Bandera para saber si el usuario está moviendo el slider de AirSpeed
let userIsSlidingAirspeed = false;
...
rollSlider.addEventListener("change", function() {
  userSliding.roll = false;
});
pitchSlider.addEventListener("change", function() {
  userSliding.pitch = false;
});
...
// Inicializar y sincronizar ambos sliders de AirSpeed
document.addEventListener("DOMContentLoaded", function() {
  updateAirspeedInstrument(40);
  const sliderCol8 = document.getElementById("airspeed-slider-col8");
  const sliderValCol8 = document.getElementById("airspeed-slider-value-col8");
  const sliderMain = document.getElementById("airspeed-slider");
  const sliderValMain = document.getElementById("airspeed-slider-value");
  function syncAirspeedSliders(val) {
    if (sliderMain) { sliderMain.value = val; sliderValMain.textContent = val; }
    if (sliderCol8) { sliderCol8.value = val; sliderValCol8.textContent = val; }
    updateAirspeedInstrument(Number(val));
  }
  if (sliderCol8) {
    sliderCol8.addEventListener("input", function() {
      userSliding.airspeed = true;
      syncAirspeedSliders(this.value);
      if(ws.readyState === 1) {
        ws.send(JSON.stringify({ setAirspeed: Number(this.value) }));
      }
    });
    sliderCol8.addEventListener("change", function() {
      userSliding.airspeed = false;
    });
  }
  if (sliderMain) {
    ...
    sliderMain.addEventListener("change", function() {
      userSliding.airspeed = false;
    });
  }
});
...
// === AirSpeed: lógica original previa (sin bandera duplicada) ===
function updateAirspeedInstrument(airspeed) {
  // Actualiza la aguja y el valor central
  let angle = airspeedToAngle(airspeed);
  document.getElementById("needle-airspeed").style.transform =
  ...
  const airspeedSliderValue = document.getElementById("airspeed-slider-value");
  const airspeedSliderCol8 = document.getElementById("airspeed-slider-col8");
  const airspeedSliderValueCol8 = document.getElementById("airspeed-slider-value-col8");
  if (airspeedSlider) { airspeedSlider.value = airspeed; airspeedSliderValue.textContent = Math.round(airspeed); }
  if (airspeedSliderCol8) { airspeedSliderCol8.value = airspeed; airspeedSliderValueCol8.textContent = Math.round(airspeed); }
}

// Eventos para ambos sliders de AirSpeed (sin bandera extra)
document.addEventListener("DOMContentLoaded", function() {
  const airspeedSlider = document.getElementById("airspeed-slider");
  const airspeedSliderCol8 = document.getElementById("airspeed-slider-col8");
  const airspeedSliderValue = document.getElementById("airspeed-slider-value");
  const airspeedSliderValueCol8 = document.getElementById("airspeed-slider-value-col8");
  function syncAirspeedSliders(val) {
    if (airspeedSlider) { airspeedSlider.value = val; airspeedSliderValue.textContent = val; }
    if (airspeedSliderCol8) { airspeedSliderCol8.value = val; airspeedSliderValueCol8.textContent = val; }
    updateAirspeedInstrument(Number(val));
  }
  if (airspeedSlider) {
  ...
  if (airspeedSliderCol8) {
    airspeedSliderCol8.addEventListener("input", function() {
      syncAirspeedSliders(this.value);
      if(ws.readyState === 1) {
        ws.send(JSON.stringify({ setAirspeed: Number(this.value) }));
      }
