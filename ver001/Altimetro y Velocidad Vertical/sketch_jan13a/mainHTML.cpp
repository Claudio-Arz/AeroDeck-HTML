...
ws.onmessage = (msg) => {
  ...
  let verticalSpeed = data.verticalSpeed ?? 0; // Si no existe, usa 0


  // Para que 0 esté arriba, 1-9 en sentido horario, cada vuelta 1000 pies
  // heading: 0-999 (pies), cada 100 = 1 división
  let angle = (heading % 1000) * 360 / 1000; // - 90;
  document.getElementById("needle").style.transform =
    `translate(-50%, -100%) rotate(${angle}deg)`;
  document.getElementById("heading-value").textContent =
    Math.round(heading);

  // La aguja de velocidad vertical parte en 270° (abajo), y se mueve +/-36° por cada unidad
  let vsAngle = 270 + verticalSpeed * 36;
  document.getElementById("needle-vs").style.transform =
    `translate(-50%, -100%) rotate(${vsAngle}deg)`;
  // Eliminado el valor y leyenda de velocidad vertical debajo de los instrumentos
