...
/*
  ...
  instrumento analógico Oil Press aeronáutico
...
int ant_Val_RPM = 0;
float vsVar = 0.0f;
float varManifold = 0.0f; // Valor del slider de giro direccional
...
void onWsEvent(uint8_t num, WStype_t type, uint8_t * payload, size_t length) {
  if (type == WStype_TEXT) {
    ...
    // Recibir valor del slider de giro direccional
    if (msg.indexOf("setManifold") >= 0) {
      int start = msg.indexOf(":");
      int end = msg.indexOf("}", start);
      if (start > 0 && end > start) {
        String val = msg.substring(start + 1, end);
        varManifold = val.toFloat();
      }
    }
...
void loop() {
  ...
  // Limitar el valor de varRPM entre 10 y 50
  if (varRPM > 50) varRPM = 50;
  if (varRPM < 10) varRPM = 10;
  ...
  if (rpm != 0.0f) {
    ...
    if (rpm_mostrar < 10) rpm_mostrar = 10;
    if (rpm_mostrar > 50) rpm_mostrar = 50;
  }
  ...
  char buffer[160];
  sprintf(buffer, "{\"rpm\":%.2f,\"varRPM\":%d,\"manifold\":%.2f}", rpm_mostrar, (int)varRPM, varManifold);
