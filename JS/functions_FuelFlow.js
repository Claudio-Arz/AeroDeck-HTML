/* 
  ...
  Librería functions_FuelFlow.js - Funciones para el instrumento de flujo de combustible (Fuel Flow).
    ...
    - updateFuelFlow(fuelFlow, noice): Actualiza la aguja y los valores numéricos del instrumento 
    según el valor de flujo de combustible recibido o el slider. 
    - initFuelFlowControls(): Inicializa las referencias a los elementos del DOM y configura 
    los event listeners para los controles interactivos del Fuel Flow, como el slider y los 
    botones de valores predefinidos.

  El instrumento de flujo de combustible muestra el flujo en tiempo real. El valor se representa en litros por hora (L/h) y se 
  muestra en una escala que va de 0 a 100 L/h. El instrumento tiene una aguja que se mueve 
  para indicar el flujo actual, y un valor numérico que muestra el flujo exacto. El control de flujo se puede ajustar mediante 
  un slider o botones predefinidos para valores comunes de flujo. Al cambiar el valor, 
...
function initFuelFlowControls() {
  ...
  if (btnMax) {
    btnMax.addEventListener('click', () => {
      updateFuelFlow(20, true); // Máximo: 20 Gls/h
    });
  }
  if (btnMid) {
    btnMid.addEventListener('click', () => {
      updateFuelFlow(10, true); // Medio: 10 Gls/h
    });
  }
  if (btnMin) {
    btnMin.addEventListener('click', () => {
      updateFuelFlow(0, true); // Mínimo: 0 Gls/h
    });
  }
  if (btnPlus) {
...
// Mapea el valor de Fuel Flow (0-20) al ángulo de la aguja (270-90 grados)
function fuelFlowToAngle(fuelFlow) {
  const minValue = 0;
  const maxValue = 20;
  const minAngle = 270; // Ángulo mínimo para 0 Gls/h
  const maxAngle = 90; // Ángulo máximo para 20 Gls/h
